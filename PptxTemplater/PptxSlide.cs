namespace PptxTemplater
{
    ...
    public class PptxSlide
    {
        ...
        /// <summary>
        /// Adds hyperlink relationship to the slide.
        /// </summary>
        /// <remarks>
        /// Hyperlinks (both internal and external) are part of the slide.
        /// You can add hyperlink to slide [run] element by its id.
        /// </remarks>
        /// <param name="hyperlinkUri">Absolute (e.g. http://) or relative (e.g. /path) URL</param>
        /// <param name="isExternal">True for Internet URLs</param>
        /// <param name="id">Unique id</param>
        /// <returns></returns>
        public string AddHyperlinkRelationship(Uri hyperlinkUri, bool isExternal, string id)
        {
            return this.slidePart.AddHyperlinkRelationship(hyperlinkUri, isExternal, id).Id;
        }

        /// <summary>
        /// Extracts http[s] urls from HTML text (looking for '<a>*</a>' tags
        /// </summary>
        /// <param name="text">HTML text</param>
        /// <returns>[unique]Key-Value pairs</returns>
        public static IDictionary<string, string> ParseHttpUrls(string text)
        {
            var mapping = new Dictionary<string, string>();
            if (text == null) { return mapping; }

            var t = DateTime.Now.Ticks.ToString(); // timestamp
            var index = 0;
            text = text?.Replace(" ", ""); // remove all spaces
            text = text?.Replace("'", "\"");

            List<string> hrefs = new List<string>();
            var startIdx = text.IndexOf("<a");
            var endIdx = text.IndexOf("</a>");
            while (startIdx >= 0 && endIdx >= 0)
            {
                hrefs.Add(text.Substring(startIdx, endIdx - startIdx));
                text = text.Substring(endIdx + 4);
                startIdx = text.IndexOf("<a");
                endIdx = text.IndexOf("</a>");
            }

            foreach (var item in hrefs)
            {
                var uri = item.Substring(item.IndexOf("href=\"") + 6);
                uri = uri.Substring(0, uri.IndexOf('"'));
                mapping.Add("hrt" + t + index, uri);
                index++;
            }
